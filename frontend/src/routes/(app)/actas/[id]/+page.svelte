...
<div class="animate-fade-in-up space-y-6">
	...
	{#if error || !acta}
		...
		<Card>
			<div class="flex flex-col items-center justify-center py-12 text-center">
				...
				<h3 class="font-heading text-lg font-bold text-neutral-900">Acta no encontrada</h3>
				<p class="mt-1.5 text-sm text-neutral-500 max-w-md">
	...
	{:else}
		...
		<div class="rounded-xl border border-neutral-200 bg-white p-5 sm:p-6 shadow-xs">
			<div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
				<div>
					<div class="flex flex-wrap items-center gap-2.5">
						<h1 class="font-heading text-xl sm:text-2xl font-bold text-neutral-900">
							{acta.consecutivo}
						</h1>
					...
					<p class="mt-1 text-sm font-medium text-neutral-700">
						Ficha: <strong class="text-neutral-900">{acta.ficha}</strong>
						{#if acta.programa}
							— <span class="text-neutral-500">{acta.programa}</span>
						{/if}
					</p>

					<p class="mt-1 text-xs text-neutral-500">
						Sesión celebrada el <strong class="font-semibold text-neutral-700">{formatDate(acta.fecha)}</strong>
					</p>
				</div>

				<div class="flex flex-wrap items-center gap-2">
					<span class="inline-flex items-center gap-1.5 rounded-lg bg-neutral-100 px-3 py-1.5 text-xs font-semibold text-neutral-700">
		...
		<Card title="Documentos y Anexos del Acta">
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				...
				<div class="flex items-start justify-between rounded-lg border border-neutral-200 bg-neutral-50/70 p-4 transition-colors hover:bg-neutral-50">
					<div class="flex items-start gap-3">
						<!-- Ícono PDF -->
						<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-danger-500/10 text-danger-600">
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
						...
						<div>
							...
							<p class="text-[11px] text-neutral-500">Documento oficial firmado · PDF (1.4 MB)</p>
						</div>
					</div>
					...
					<div class="flex items-center gap-1.5">
						<button
							...
							class="rounded-md border border-neutral-300 bg-white px-2.5 py-1 text-xs font-semibold text-neutral-700 hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 cursor-pointer"
						>
							Vista previa
						</button>
						<button
				...
				<div class="flex items-start justify-between rounded-lg border border-neutral-200 bg-neutral-50/70 p-4 transition-colors hover:bg-neutral-50">
					<div class="flex items-start gap-3">
						<!-- Ícono Excel -->
						<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-success-500/10 text-success-600">
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
						...
						<div>
							...
							<p class="text-[11px] text-neutral-500">Matriz de seguimiento formativo · Excel (480 KB)</p>
						</div>
					</div>
		...
		<Card title="Casos Evaluados en la Sesión ({casos.length})">
			...
			{:else}
						...
						<tbody class="divide-y divide-neutral-200 text-neutral-800">
							{#each casos as caso (caso.id)}
								<tr class="transition-colors hover:bg-neutral-50/60">
									<td class="px-4 py-3 align-top font-medium text-neutral-900 whitespace-nowrap">
										<div>{caso.aprendiz}</div>
										{#if caso.documento}
											<div class="text-xs text-neutral-400 font-normal">C.C. {caso.documento}</div>
										{/if}
									</td>
									...
									<td class="px-4 py-3 align-top text-xs text-neutral-600 leading-relaxed max-w-md">
										<p>{caso.descripcion}</p>
										{#if caso.sancion_sugerida}
											<p class="mt-1 text-[11px] font-medium text-neutral-800">
												<strong>Medida acordada:</strong> {caso.sancion_sugerida}
											</p>
										{/if}
									</td>
									<td class="px-4 py-3 align-top whitespace-nowrap">
										<span class="inline-flex rounded-md bg-neutral-100 px-2 py-1 text-xs font-semibold text-neutral-700">
											{caso.estado}
										</span>
									</td>
								</tr>
				...
				<div class="sm:hidden space-y-3">
					{#each casos as caso (caso.id)}
						<div class="rounded-lg border border-neutral-200 bg-neutral-50/50 p-3.5 space-y-2">
							<div class="flex items-start justify-between gap-2">
								<div>
									<p class="text-sm font-bold text-neutral-900">{caso.aprendiz}</p>
									{#if caso.documento}
										<p class="text-[11px] text-neutral-500">C.C. {caso.documento}</p>
									{/if}
								</div>
								<Badge variant={tipoCasoVariant[caso.tipo] ?? 'neutral'}>
									{caso.tipo}
							...
							{#if caso.sancion_sugerida}
								<div class="rounded bg-white p-2 text-[11px] text-neutral-700 border border-neutral-200">
									<strong>Medida:</strong> {caso.sancion_sugerida}
								</div>
							{/if}

							<div class="pt-1 flex items-center justify-between text-xs">
								<span class="text-neutral-500 font-medium">Estado:</span>
								<span class="font-semibold text-neutral-800">{caso.estado}</span>
							</div>
						</div>
					{/each}
				</div>
...
{#if modalPreviewAbierto}
	...
	>
		<div class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl space-y-4">
			<div class="flex items-center justify-between border-b border-neutral-200 pb-3">
				<div>
					<h3 class="font-heading text-base font-bold text-neutral-900">
						Vista Previa: {acta?.consecutivo}
					</h3>
					<p class="text-xs text-neutral-500">Documento oficial emitido por el Comité CSET</p>
				</div>
				<button
					type="button"
					onclick={() => modalPreviewAbierto = false}
					class="rounded-lg p-1 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-700 cursor-pointer"
			...
			<div class="h-80 overflow-y-auto rounded-lg border border-neutral-200 bg-neutral-50 p-6 text-xs text-neutral-700 space-y-3 font-mono">
				<div class="text-center font-bold font-sans border-b border-neutral-300 pb-2">
				...
				<div class="border-t border-neutral-200 pt-2 font-sans">
					<p class="font-semibold mb-1">RESUMEN EJECUTIVO DE LA SESIÓN:</p>
					<p class="text-neutral-600 leading-relaxed font-sans">
